...
L.Control.Coordinates = L.Control.extend({
	...
	_createCoordinateLabel: function(ll) {
		...
		if (opts.labelLng) {
			...
			x = L.Util.template(opts.labelTemplateLng, {
				x: this._getNumber(ll.lng, opts, "lng")
			});
		}
		if (opts.labelFormatterLat) {
			...
			y = L.Util.template(opts.labelTemplateLat, {
				y: this._getNumber(ll.lat, opts, "lat")
			});
		}
	 ...
	 */
	_getNumber: function(n, opts, coord) {
		var res;
		if (opts.useDMS) {
			res = L.NumberFormatter.toDMS(n, coord);
		} else if (opts.useDMD) {
			res = L.NumberFormatter.toDMD(n, coord);
		} else {
			res = L.NumberFormatter.round(n, opts.decimals, opts.decimalSeperator, coord);
		}
		return res;
	},
...
L.NumberFormatter = {
	round: function(num, dec, sep, coord) {
		var res = L.Util.formatNum(Math.abs(num), dec) + "",
			numbers = res.split(".");
		if (numbers[1]) {
	...
	toDMD: function (deg, coord) {
		var dir = "";
		var dd = Math.abs(deg);
        var d = Math.floor(dd);
        var m = (Math.round((dd - d) * 60* 100)/100).toFixed(2);
        if (m < 10) {
            m = "0" + m;
        }
        if (d < 10) {
